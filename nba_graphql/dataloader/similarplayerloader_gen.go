...
type SimilarPlayerLoaderConfig struct {
	...
	Fetch func(keys []*model.SimilarPlayerQuery) ([][]model.Player, []error)
...
type SimilarPlayerLoader struct {
	...
	cache map[*model.SimilarPlayerQuery][]model.Player
...
func (l *SimilarPlayerLoader) Load(key *model.SimilarPlayerQuery) ([]model.Player, error) {
...
func (l *SimilarPlayerLoader) LoadThunk(key *model.SimilarPlayerQuery) func() ([]model.Player, error) {
	l.mu.Lock()
...
func (l *SimilarPlayerLoader) LoadAll(keys []*model.SimilarPlayerQuery) ([][]model.Player, []error) {
...
// This method should be used if you want one goroutine to make requests to many
// different data loaders without blocking until the thunk is called.
func (l *SimilarPlayerLoader) LoadAllThunk(keys []*model.SimilarPlayerQuery) func() ([][]model.Player, []error) {
	results := make([]func() ([]model.Player, error), len(keys))
	for i, key := range keys {
		results[i] = l.LoadThunk(key)
	}
...
func (l *SimilarPlayerLoader) Prime(key *model.SimilarPlayerQuery, value []model.Player) bool {
	l.mu.Lock()
	var found bool
...
func (l *SimilarPlayerLoader) Clear(key *model.SimilarPlayerQuery) {
	l.mu.Lock()
	delete(l.cache, key)
	l.mu.Unlock()
}

func (l *SimilarPlayerLoader) unsafeSet(key *model.SimilarPlayerQuery, value []model.Player) {
	if l.cache == nil {
		l.cache = map[*model.SimilarPlayerQuery][]model.Player{}
	}
	l.cache[key] = value
}
...
func (b *similarPlayerLoaderBatch) keyIndex(l *SimilarPlayerLoader, key *model.SimilarPlayerQuery) int {
