...
const Playercard = (props) => {
  ...
  const matchups = seasonData.filter((game) => game.opponent.teamID === projection.opponent.teamID);
  ...
  return (
    <div className="playercard">
        <PlayerContext player={player} opponent={projection.opponent}></PlayerContext>
        <StatSelectBtns predictions={predictions} playername={player.name} selected={stat} onStatSelect={onStatSelect}></StatSelectBtns>
        <Prediction predictions={predictions} selected={stat} game={game}></Prediction>
        <PlayerStatsPreview predictions={predictions} selected={stat} matchups={matchups} similarData={""}></PlayerStatsPreview>
    </div>
  )
}
