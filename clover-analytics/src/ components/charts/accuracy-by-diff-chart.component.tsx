...
export interface ModelAccuracyByDiffProps {
...
export const ModelAccuracyByDiff: React.FC<ModelAccuracyByDiffProps> = ({
  propositions,
  absolute = false,
  stepSize = 3,
  steps = 5,
}: ModelAccuracyByDiffProps) => {
  ...
  ranges.forEach((x, i) => {
    const filtered = propositions.filter((prop) => {
      const pctDiff = absolute
        ? +(prop.prediction.estimation - prop.target).toFixed(2)
        : +(
            ((prop.prediction.estimation - prop.target) / prop.target) *
            100
          ).toFixed(2);
    ...
    let label = `${x.min}${!absolute ? "%" : ""} to ${x.max}${
      !absolute ? "%" : ""
    }`;
    if (x.min === -Infinity) {
      label = `<= ${x.max}%`;
